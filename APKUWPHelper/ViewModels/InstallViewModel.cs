...
namespace APKUWPHelper.ViewModels
{
    public class InstallViewModel : INotifyPropertyChanged, IDisposable
    {
        ...
        private readonly ResourceLoader _loader = ResourceLoader.GetForCurrentView();//
           //ResourceLoader.GetForViewIndependentUse("MainWindows");

        public string InstallFormat => "InstallFormat";
        public string VersionFormat => "VersionFormat";
        public string PackageNameFormat => "PackageNameFormat";
        ...
        private string _openAPKFileText;
        public string OpenAPKFileText
        {
            get => _openAPKFileText;
            set
            {
                _openAPKFileText = value;
                RaisePropertyChangedEvent();
            }
        }
        ...
        public InstallViewModel(string Path, MainWindows Page)
        {
            _path = Path;
            _page = Page;

            OpenAPKFileText = "OpenAPKFile";
        ...
        public void download()
        {
            ...
            client.DownloadDataCompleted += (sender, e) =>
            {
                try
                {
                    string zipFilePath = System.IO.Path.Combine(ApplicationData.Current.LocalFolder.Path, "platform-tools.zip");
                    byte[] data = e.Result;
            ...
            client.DownloadDataAsync(new Uri("https://dl.google.com/android/repository/platform-tools-latest-windows.zip?hl=zh-cn"));

        }
        public async Task DownloadADB()
        {
        
            WaitProgressText = "UnzipADB";
            await Task.Delay(1);
            ...
            foreach (IArchiveEntry entry in archive.Entries)
            {
                ...
                WaitProgressText = string.Format("UnzippingFormat", archive.Entries.ToList().IndexOf(entry) + 1, archive.Entries.Count());
            ...
            WaitProgressText = "UnzipComplete";
        }

        public async Task InitilizeADB()
        {
            WaitProgressText = "CheckingADB";
            await CheckADB();
            if (!string.IsNullOrEmpty(_path))
            {
                WaitProgressText = "StartingADB";
        ...
        public async Task InitilizeUI()
        {
            if (!string.IsNullOrEmpty(_path))
            {
                ...
                WaitProgressText = "Finished";
            }
            else
            {
                ResetUI();
                ApkInfo = new ApkInfo();
                AppName = "NoPackageWranning";
        ...
        public void CheckAPK()
        {
            ...
            {
                ActionButtonText = "Install";
                AppName = string.Format("InstallFormat", ApkInfo.AppName);
                ActionVisibility = LaunchWhenReadyVisibility = Visibility.Visible;
            }
            else if (info.VersionCode < int.Parse(ApkInfo.VersionCode))
            {
                ActionButtonText = "Update";
                AppName = string.Format("UpdateFormat", ApkInfo.AppName);
                ActionVisibility = LaunchWhenReadyVisibility = Visibility.Visible;
            }
            else
            {
                ActionButtonText = "Reinstall";
                SecondaryActionButtonText = "Launch";
                AppName = string.Format("ReinstallFormat", ApkInfo.AppName);
                TextOutput = string.Format("ReinstallOutput", ApkInfo.AppName);
        ...
        public async void InstallAPP()
        {
            ...
            {
                IsInstalling = true;
                CancelOperationButtonText = "Cancel";
